...
--
type family IsDP (dpDefinition :: Type) :: Bool where
...
mkFilter :: forall dpDefinition filterState filterParam st. 
            WithFilter dpDefinition filterParam (Relude.StateT filterState (DP st)) -- ^Associated type family to Generate Function Signature
...
single :: forall dpDefinition filterState filterParam st. 
          WithFilter dpDefinition filterParam (Relude.StateT filterState (DP st)) -- ^Associated type family to Generate Function Signature
       -> NonEmpty (Actor dpDefinition filterState filterParam (Relude.StateT filterState (DP st)))
single = one . actor

-- | Smart Constructor of 'Actor'.
{-# INLINE actor #-}
actor :: forall dpDefinition filterState filterParam st.
         (MonadState filterState (StateT filterState (DP st))) =>
...
actor = Actor . mkStage' @(WithFilter dpDefinition filterParam (Relude.StateT filterState (DP st)))
...
(|>>>) :: forall dpDefinition filterState filterParam st. 
          Actor dpDefinition filterState filterParam (Relude.StateT filterState (DP st)) -- ^New 'Actor' to put on front
...
{-# INLINE (|>>) #-}
(|>>) :: forall dpDefinition filterState filterParam st. 
         Actor dpDefinition filterState filterParam (Relude.StateT filterState (DP st)) -- ^'Actor' 1
      -> Actor dpDefinition filterState filterParam (Relude.StateT filterState (DP st)) -- ^'Actor' 2
      -> Filter dpDefinition filterState filterParam st
...
type SpawnFilterConstraint dpDefinition readElem st filterState filterParam l l1 l2 l3 b2 b3 l4 =
                ...
                , WithFilter dpDefinition filterParam (Relude.StateT filterState (DP st)) ~ (b2 -> ReadChannel b2 -> b3)
                , HLength (ExpandFilterToCh dpDefinition filterParam) ~ HLength l2
                , HCurry' (HLength l2) (WithFilter dpDefinition filterParam (Relude.StateT filterState (DP st))) l2 (Relude.StateT filterState (DP st) ())
                , ArityFwd (WithFilter dpDefinition filterParam (Relude.StateT filterState (DP st))) (HLength (ExpandFilterToCh dpDefinition filterParam))
...
unfoldF :: forall dpDefinition readElem st filterState filterParam l l1 l2 l3 b2 b3 l4.
        ...
        -> DP st (HList l) 
-}
unfoldF :: forall k dpDefinition readElem (st :: k) filterState
                                     filterParam (l :: [*]) (l1 :: [*]) (l2 :: [*]) (l3 :: [*]) b2
                                     b3 (l4 :: [*]).
           SpawnFilterConstraint dpDefinition readElem st filterState filterParam l l1 l2 l3 b2 b3 l4
        => UnFoldFilter dpDefinition readElem st filterState filterParam l 
        -> DP st (HList l) 
